...
import { Portfolio, OwnedStock } from 'src/app/core/models/portfolio/portfolio';
...
export class PortfolioService {
  ...
  getOwnedStock(stockId: string): Promise<OwnedStock> {
    return new Promise((resolve, reject) => {
      this.http.get(`${environment.apiBase}/portfolio/ownedstock/${stockId}`)
        .subscribe((res: ClientResponse) => {
          resolve(res.result.ownedStock)
        },
          (err: { error: ClientResponse }) => reject(err.error))
    })
  }
